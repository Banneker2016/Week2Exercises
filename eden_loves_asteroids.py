...
def near_earth():
    L = [t[0] for t in Asteroids if abs(t[1]-1)<0.2 and t[2]<0.5]
    for name in L:
        print name
    print ''
...
#creating dictionary
def dictionary():
    print 'Would you like to organize by name, semi-major axis (in AU), eccentricity, or orbit class?'
    response = raw_input('>>> ')
    new_list = []
    D = {t[0]: (t[1], t[2], orbit_class[t[3]]) for t in Asteroids}
    dictionary = {}
    if response == 'name':
        dictionary = sorted(D.items(), key=lambda (k,v): k)
        return dictionary
    elif response == 'semi-major axis':
        dictionary = sorted(D.items(), key=lambda (k,v): v[0])
        return dictionary
    elif response == 'eccentricity':
        dictionary = sorted(D.items(), key=lambda (k,v): v[1])
        return dictionary
    elif response == 'orbit class':
        dictionary = sorted(D.items(), key=lambda (k,v): v[2])
        return dictionary
    else:
        pass

def organize():
    data = [[t[0], str(t[1]), str(t[2]), orbit_class[t[3]]] for t in Asteroids]
    col_width = max(len(val) for row in data for val in row) + 2  # padding
    f = open("asteroids.txt", 'w')
    
    for row in data:
        f.write("".join(val.ljust(col_width) for val in row) + '\n')
